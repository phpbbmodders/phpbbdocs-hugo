# Language TODOs

Index of per-language TODO directories under `docs/TODO/`. Each
language's own `README.md` links to its detailed TODOs (terminology
audit, translation review notes, supplementary glossary, etc.).

- [German (de-x-sie)](de-x-sie/README.md)
- [German (de)](de/README.md)
- [French (fr)](fr/README.md)
- [Danish (da)](da/README.md)

A language with no outstanding TODOs (currently English) has no entry
here — only languages with an active `docs/TODO/<code>/` directory are
listed. Add a new language's directory to this list as soon as it gets
its first detailed TODO.
