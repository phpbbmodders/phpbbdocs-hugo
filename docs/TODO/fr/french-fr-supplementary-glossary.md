...
| Recompile stale templates | Recompiler les gabarits obsolètes | admin_guide.xml:589 | Same pre-existing English-source issue flagged in the German glossary — closest real key is about "style components," not templates. | | | |
...
| Maximum thumbnail width/filesize | Largeur maximale de vignette en pixels / Taille de fichier maximale de la vignette | admin_guide.xml:1057,1058 | No candidate key found (parallel to the German glossary's thumbnail-filesize entry). | | | |
