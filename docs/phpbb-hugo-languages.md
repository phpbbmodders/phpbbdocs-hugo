...
- **Italian** — `it` in this project, phpBB's **Italian** pack, Hugo locale `it-IT` (see row above). No honorific split (single register). Screenshots reuse the English set, same reasoning as French. `content/it/` (all seven end-user chapters) and `dev-docs-docbook/it/` (all 55 developer-docs files) are both done.
